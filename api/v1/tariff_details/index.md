...
Get unit prices, subscription fees and other meta data of one or many tariffs, given a specific CPO or charging station.
...
| station.id                       | Array<String> | optional*    | `["some-uuid"]`                        | Return prices for the given station IDs. (Stations can be fetched via the [Charging Stations API](../charging_stations/index.md)). If not provided, only CPO based prices will be returned. Max. number of IDs: 50                                                                                                                                                                                           |
...
| filter.all_weekdays              | Boolean       | optional     | `true`                                 | If `false`, for segments with a weekday restriction only the segment matching the current weekday is returned. If `true`, one `restricted_segment` per weekday is returned instead (see `restricted_segments.weekdays`). Default: `false`.                                                                                                                                                                   |
...
| restricted_segments.time_of_day_start           | Integer         | 720                 | Time of day when this segment starts to match. In minutes since midnight.                                                                                                                                                                                                                                                                                                                                             |
| restricted_segments.time_of_day_end             | Integer         | 1200                | Time of day when this segment stops to match. In minutes since midnight.                                                                                                                                                                                                                                                                                                                                              |
...
| restricted_segments.weekdays                    | Array<String> or null | `["mon"]`     | Weekdays this segment applies to, if the tariff has weekday-specific pricing. Array of one or more of `mon`, `tue`, `wed`, `thu`, `fri`, `sat`, `sun`. `null` if the segment has no weekday restriction. Always present, but only split out per weekday if `filter.all_weekdays` is `true` - otherwise the array only contains the current weekday (if the segment has a weekday restriction).                     |
...
| tariff.is_secondary_tariff        | Boolean            | `false`                                         | This tariff is not the main offer of the EMP (e.g. a white label for a partner, or a regional offer).                                                                                                                                                                                                                                                                                                                    |
